...
class TwitterScraper(BaseScraper):
	class ApiAuth(object):
		"""Container for the authentication information needed to access twitter's API."""
	...
	def get_followers(self,id_or_username):
		...
		# Call the appropriate internal method based on whether API auth information exists and whether the rate limit is reached.
		followers = []
		cursor = 0
		if self.api_auth:
			while True:
				try:
					f,cont = self._get_followers_api(id_or_username,is_id=is_id,cursor=cursor)
				...
				# if we are continuing, then the number of followers returned has to equal the pagination limit
				# of the API - otherwise, something is wrong!
				assert len(followers) == TwitterScraper.ApiAuth._api_limits["get_followers_pagination"]
				cursor += TwitterScraper.ApiAuth._api_limits["get_followers_pagination"]

		followers += self._get_followers_noapi(id_or_username,is_id=is_id,cursor=cursor)
		return followers
		
	def _get_followers_api(self,id_or_username,is_id,cursor=-1):
		"""Get the followers of a particular user using the stored twitter API info.
		Returns a chunk of followers starting at the given cursor position
		May throw RateLimitError.
		"""
		pass # return f=[...],cont=t/f

	def _get_followers_noapi(self,id_or_username,is_id,cursor=-1):
		"""Get the followers of a particular user by scraping, starting at the given cursor position."""
		pass # return f=[...]